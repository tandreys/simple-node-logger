...
const JSONFileAppender = require( './JSONFileAppender' );
...
const SimpleLogger = function(opts) {
    ...
    /**
     * create a json file appender and add it to the appender list
     *
     * @param opts
     * @returns the appender
     */
    this.createJSONFileAppender = function( opts ) {
        return manager.addAppender( new JSONFileAppender( opts ) );
    };
...
/**
 * static convenience method to create a JSONFile logger with console
 *
 * @param options - optional, if present then it could be 1) a string or 2) and object.  if it's a string it's assumed
 * to be the logFilePath; if it's a string or an object with logFilePath property, then a file appender is created.
 *
 * Valid options:
 *  - logFilePath : a path to the JSONfile appender
 *  - domain : the logger domain, e.g., machine or site id
 *  - dfltLevel : the default log level (overrides info level)
 *  - timestampFormat : the format used for log entries (see moment date formats for all possibilities)
 *
 * @returns logger
 */
SimpleLogger.createSimpleJSONFileLogger = function(options) {
    'use strict';
    ...
    if (opts.logFilePath) {
        manager.createJSONFileAppender( opts );
    }
